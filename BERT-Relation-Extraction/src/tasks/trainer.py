...
from src.tasks.preprocessing_funcs import Pad_Sequence, semeval_dataset
...
def train_and_fit(args):
    ...
    for run, (train_index, test_index) in enumerate(kf.split(numpy_data)):
        ...
        train_set = semeval_dataset(
            pd.DataFrame(train, columns=["sents", "relations", "relations_id"]),
            tokenizer=tokenizer,
            e1_id=e1_id,
            e2_id=e2_id,
        )
        test_set = semeval_dataset(
            pd.DataFrame(test, columns=["sents", "relations", "relations_id"]),
            tokenizer=tokenizer,
            e1_id=e1_id,
            e2_id=e2_id,
        )
        ...
        for epoch in range(start_epoch, args.num_epochs):
            ...
            print(
                "Train accuracy at Epoch %d: %.7f" % (epoch + 1, accuracy_per_epoch[-1])
            )
            ...
            if accuracy_per_epoch[-1] > best_pred:
                ...
                torch.save(
                    ...
                    os.path.join(
                        "./data/", "task_test_model_best_%d.pth.tar" % args.model_no
                    ),
                )
                f1_scores_per_split[run] = results["f1"]
            ...
            if (epoch % 1) == 0:
                ...
                save_as_pickle(
                    "task_test_f1_per_epoch_%d.pkl" % args.model_no, test_f1_per_epoch
                )
                torch.save(
                    {
                        "epoch": epoch + 1,
                    ...
                    os.path.join(
                        "./data/", "task_test_checkpoint_%d.pth.tar" % args.model_no
                    ),
                )
    ...
    # plt.savefig(
    #     os.path.join("./data/", "task_train_accuracy_vs_epoch_%d.png" % args.model_no)
    # )
